...
COL = bpy.data.collections.new('FIRIPU_BLOCKOUT_V02')
...
for side,x in [('L',-0.105),('R',0.105)]:
    ...
    cube(f'Lengueta_{side}', (x,-0.155,0.105), (0.045,0.012,0.052), BOOT, 0.012)
    for lace_i in range(3):
        z = 0.078 + lace_i * 0.025
        cylinder_between(f'Cordon_{side}_{lace_i}', (x-0.045,-0.168,z), (x+0.045,-0.168,z+0.012), 0.004, CREAM, 10)
...
cylinder_between('Cordon_Poleron_L', (-0.035,-0.143,0.82), (-0.035,-0.15,0.735), 0.005, BLUE, 10)
cylinder_between('Cordon_Poleron_R', (0.035,-0.143,0.82), (0.035,-0.15,0.735), 0.005, BLUE, 10)
uv('Tope_Cordon_L', (-0.035,-0.15,0.725), (0.008,0.006,0.014), BLUE, 14, 8)
uv('Tope_Cordon_R', (0.035,-0.15,0.725), (0.008,0.006,0.014), BLUE, 14, 8)
...
for side,s in [('L',-1),('R',1)]:
    ...
    palm=(s*0.347,-0.005,0.486)
    uv(f'Palma_{side}', palm, (0.045,0.032,0.058), SKIN, 24, 14)
    finger_lengths=[0.042,0.054,0.059,0.054,0.043]
    finger_offsets=[-0.030,-0.015,0.0,0.015,0.030]
    for finger_i,(length,yoff) in enumerate(zip(finger_lengths,finger_offsets)):
        fx=s*(0.349 + (finger_i-2)*0.007)
        top=(fx,-0.010+yoff,0.459)
        bottom=(fx,-0.010+yoff,0.459-length)
        cylinder_between(f'Dedo_{side}_{finger_i}', top, bottom, 0.009 if finger_i else 0.010, SKIN, 12)
        uv(f'PuntaDedo_{side}_{finger_i}', bottom, (0.010,0.010,0.012), SKIN, 14, 8)
    cylinder_between(f'Pulgar_{side}', (s*0.323,-0.012,0.493), (s*0.306,-0.018,0.454), 0.011, SKIN, 12)
...
# Labios y mentón neutral
uv('Mentón', (0,-0.133,0.865), (0.052,0.025,0.036), SKIN, 22, 12)
cube('Labio_Superior', (0,-0.165,0.892), (0.034,0.005,0.004), BOOT, 0.005)
cube('Labio_Inferior', (0,-0.166,0.883), (0.029,0.005,0.004), SKIN, 0.005)
...
scene.render.filepath=os.path.join(ROOT,'firipu_blockout_v02_preview.png')
scene.render.image_settings.color_mode='RGBA'

# Guardar y exportar exclusivamente la colección Firipu.
blend_path=os.path.join(ROOT,'firipu_blockout_v02.blend')
bpy.ops.wm.save_as_mainfile(filepath=blend_path)
bpy.ops.object.select_all(action='DESELECT')
for o in COL.objects: o.select_set(True)
bpy.context.view_layer.objects.active=next(iter(COL.objects))
glb_path=os.path.join(ROOT,'firipu_blockout_v02.glb')
