...
# Mundo 8 — Maule: Ríos y Robles del Maule.
# Mecánica regional: tronco flotante temporal.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $LoroTricahueRobot
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Maule: Ríos y Robles iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Maule: usa troncos flotantes para cruzar ríos, registra 4 especies y enfrenta al Loro Tricahue Robot.")
	print("FLOW_START Mundo 8 Maule")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 8 Maule")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 8 Maule")
	get_tree().quit(0)
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("maule")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 8 Maule")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 8 Maule")
	if hud.has_method("show_message"):
		hud.show_message("¡Loro Tricahue Robot detenido! El Maule fluye libre. Toma la medalla.")
