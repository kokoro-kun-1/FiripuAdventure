...
# Mundo 2 — Ñuble Cordillerano (Nevados de Chillán).
# Reutiliza el flujo del Biobío: 4 especies, jefe robot, medalla.
# Mecánica regional propia: vapor termal como ascensor temporal.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $HuemulGuardian
...
func _ready() -> void:
    ...
    _start_world_flow()
    print("Firipu Adventure — Ñuble Cordillerano iniciado")
...
func _start_world_flow() -> void:
    ...
    if hud.has_method("show_message"):
        hud.show_message("Ñuble: usa los chorros de vapor termal para subir, registra 4 especies y enfrenta al guardián.")
    print("FLOW_START Mundo 2 Ñuble")

func _on_start_requested() -> void:
    gameplay_started = true
    if player.has_method("set_input_locked"):
        player.set_input_locked(false)
    print("FLOW_GAMEPLAY_START Mundo 2 Ñuble")
...
func _on_exit_requested() -> void:
    print("FLOW_EXIT_REQUESTED Mundo 2 Ñuble")
    get_tree().quit(0)
...
func _on_medal_area_body_entered(body: Node) -> void:
    if body == player and int(player.get("collected")) >= 4 and bool(boss.get("defeated")) and player.has_method("obtain_medal"):
        ...
        hud.show_message("La medalla se libera al completar el Diario y reparar al guardián.")
...
func _on_prototype_completed() -> void:
    ...
    if global_progress != null:
        global_progress.complete_world("nuble")
        global_progress.force_save()
    print("FLOW_VICTORY Mundo 2 Ñuble")
    _on_save_requested()
...
# Yuki detecta el punto debil del guardián en la fase de núcleo.
...
func _on_boss_defeated() -> void:
    print("FLOW_BOSS_DEFEATED Mundo 2 Ñuble")
    if hud.has_method("show_message"):
        hud.show_message("¡Guardián reparado! Ñuble está en calma. Toma la medalla.")
