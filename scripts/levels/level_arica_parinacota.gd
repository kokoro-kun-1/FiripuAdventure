...
# Mundo 16 — Arica y Parinacota: Altiplano de Arica y Parinacota.
# Mecánica: salto viento altiplánico (saltos largos con viento).

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $PicaflorCosmico
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Arica y Parinacota: Altiplano iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Arica y Parinacota: usa saltos de viento altiplánico, registra 4 especies y enfrenta al Picaflor Cósmico.")
	print("FLOW_START Mundo 16 Arica y Parinacota")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 16 Arica y Parinacota")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 16 Arica y Parinacota")
	get_tree().quit(0)
...
func _on_medal_area_body_entered(body: Node) -> void:
	if body == player and int(player.get("collected")) >= 4 and bool(boss.get("defeated")) and player.has_method("obtain_medal"):
		...
		hud.show_message("La medalla se libera al completar el Diario y calmar al Picaflor Cósmico.")
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("arica_parinacota")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 16 Arica y Parinacota")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 16 Arica y Parinacota")
	if hud.has_method("show_message"):
		hud.show_message("¡Picaflor Cósmico calmado! El altiplano brilla en paz. Toma la medalla.")
