...
# Mundo 9 — O'Higgins: Quebradas de O'Higgins.
# Mecánica: eco tricahue revela rutas ocultas.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $TricahueRobot
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — O'Higgins: Quebradas iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("O'Higgins: usa el eco tricahue para revelar rutas, registra 4 especies y enfrenta al Tricahue Robot.")
	print("FLOW_START Mundo 9 O'Higgins")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 9 O'Higgins")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 9 O'Higgins")
	get_tree().quit(0)
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("ohiggins")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 9 O'Higgins")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 9 O'Higgins")
	if hud.has_method("show_message"):
		hud.show_message("¡Tricahue Robot calmado! Las quebradas cantan en paz. Toma la medalla.")
