...
# Mundo 10 — Metropolitana: Cerros de la Capital Natural.
# Mecánica: impulso cerro para descensos controlados.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $DeguRobot
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Metropolitana: Cerros Capital Natural iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Metropolitana: usa impulso cerro para descensos, registra 4 especies y enfrenta al Degú Robot.")
	print("FLOW_START Mundo 10 Metropolitana")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 10 Metropolitana")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 10 Metropolitana")
	get_tree().quit(0)
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("metropolitana")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 10 Metropolitana")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 10 Metropolitana")
	if hud.has_method("show_message"):
		hud.show_message("¡Degú Robot reparado! Los cerros isla respiran tranquilos. Toma la medalla.")
