...
# Mundo 6 — Aysén: Viento Patagónico de Aysén.
# Reutiliza el flujo: 4 especies, jefe robot, medalla.
# Mecánica regional propia: impulso viento patagónico para saltos largos controlados.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $HuemulGlacial
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Aysén: Viento Patagónico iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Aysén: usa el viento patagónico para saltos largos, registra 4 especies y enfrenta al Huemul Glacial.")
	print("FLOW_START Mundo 6 Aysén")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 6 Aysén")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 6 Aysén")
	get_tree().quit(0)
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("aysen")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 6 Aysén")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 6 Aysén")
	if hud.has_method("show_message"):
		hud.show_message("¡Huemul Glacial reparado! La Patagonia respira en calma. Toma la medalla.")
