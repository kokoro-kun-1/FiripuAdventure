...
# Mundo 11 — Valparaíso: Acantilados de Valparaíso.
# Mecánica: cometa costera para planear con viento costero.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $PicaflorRobot
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Valparaíso: Acantilados iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Valparaíso: usa cometa costera para planear, registra 4 especies y enfrenta al Picaflor Robot.")
	print("FLOW_START Mundo 11 Valparaíso")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 11 Valparaíso")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 11 Valparaíso")
	get_tree().quit(0)
...
func _on_prototype_completed() -> void:
	...
	if global_progress != null:
		global_progress.complete_world("valparaiso")
		global_progress.force_save()
	print("FLOW_VICTORY Mundo 11 Valparaíso")
	_on_save_requested()
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 11 Valparaíso")
	if hud.has_method("show_message"):
		hud.show_message("¡Picaflor Robot calmado! Los acantilados recuperan su brisa. Toma la medalla.")
