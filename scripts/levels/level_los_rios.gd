...
# Mundo 4 — Los Ríos: Selva Lluviosa de Los Ríos.
# Reutiliza el flujo del Biobío/Ñuble/Araucanía: 4 especies, jefe robot, medalla.
# Mecánica regional propia: hoja paraguas para planear bajo lluvia.

@onready var player: Node = $Firipu
@onready var hud: Node = $HUD
@onready var medal_area: Area3D = $MedalArea
@onready var boss: Node = $CaracolBlindado
...
func _ready() -> void:
	...
	_start_world_flow()
	print("Firipu Adventure — Los Ríos: Selva Lluviosa iniciado")
...
func _start_world_flow() -> void:
	...
	if hud.has_method("show_message"):
		hud.show_message("Los Ríos: usa hojas paraguas para planear bajo la lluvia, registra 4 especies y enfrenta al Caracol Blindado.")
	print("FLOW_START Mundo 4 Los Ríos")

func _on_start_requested() -> void:
	gameplay_started = true
	if player.has_method("set_input_locked"):
		player.set_input_locked(false)
	print("FLOW_GAMEPLAY_START Mundo 4 Los Ríos")
...
func _on_exit_requested() -> void:
	print("FLOW_EXIT_REQUESTED Mundo 4 Los Ríos")
	get_tree().quit(0)
...
func _on_boss_defeated() -> void:
	print("FLOW_BOSS_DEFEATED Mundo 4 Los Ríos")
	if hud.has_method("show_message"):
		hud.show_message("¡Caracol Blindado reparado! Los Ríos recuperan su calma. Toma la medalla.")
