...
# Yuki detecta el punto debil del jefe y expone su nucleo.
func detect_weakness(target: Node) -> void:
    if target == null:
        return
    if target.has_method("expose_core"):
        target.expose_core()
        print("%s detecta el punto debil del jefe" % companion_role)
