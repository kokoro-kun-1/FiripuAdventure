...
const TEST_NAME: String = "METROPOLITANA_TEST"
const METROPOLITANA_SCENE := "res://scenes/levels/level_metropolitana.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, METROPOLITANA_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var impulso_a: Node = level.get_node_or_null("ImpulsoCerroA")
	var impulso_b: Node = level.get_node_or_null("ImpulsoCerroB")
	var impulso_c: Node = level.get_node_or_null("ImpulsoCerroC")
	var boss: Node = level.get_node_or_null("DeguRobot")
	...
	if kira == null:
		...
		return
	if impulso_a == null or impulso_b == null or impulso_c == null:
		TestUtils.fail(self, TEST_NAME, "Impulsos cerro not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "DeguRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar impulso cerro activa hill_boost
	if not firipu.has_method("activate_hill_boost"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_hill_boost")
		return

	impulso_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("hill_boost_active")):
		TestUtils.fail(self, TEST_NAME, "hill_boost not activated after interacting with impulso")
		return
