...
const TEST_NAME: String = "LOS_LAGOS_TEST"
const LOS_LAGOS_SCENE := "res://scenes/levels/level_los_lagos.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, LOS_LAGOS_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var balsa_a: Node = level.get_node_or_null("BalsaNaturalA")
	var balsa_b: Node = level.get_node_or_null("BalsaNaturalB")
	var balsa_c: Node = level.get_node_or_null("BalsaNaturalC")
	var boss: Node = level.get_node_or_null("HuillinRobot")
	...
	if kira == null:
		...
		return
	if balsa_a == null or balsa_b == null or balsa_c == null:
		TestUtils.fail(self, TEST_NAME, "Balsas naturales not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "HuillinRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar balsa natural activa raft
	if not firipu.has_method("activate_raft"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_raft")
		return

	balsa_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("raft_active")):
		TestUtils.fail(self, TEST_NAME, "raft not activated after interacting with balsa")
		return
