...
const TEST_NAME: String = "AYSEN_TEST"
const AYSEN_SCENE := "res://scenes/levels/level_aysen.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, AYSEN_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var viento_a: Node = level.get_node_or_null("ImpulsoVientoA")
	var viento_b: Node = level.get_node_or_null("ImpulsoVientoB")
	var viento_c: Node = level.get_node_or_null("ImpulsoVientoC")
	var boss: Node = level.get_node_or_null("HuemulGlacial")
	...
	if kira == null:
		...
		return
	if viento_a == null or viento_b == null or viento_c == null:
		TestUtils.fail(self, TEST_NAME, "Impulsos viento not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "HuemulGlacial boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar impulso viento activa wind_boost
	if not firipu.has_method("activate_wind_boost"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_wind_boost")
		return

	viento_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("wind_boost_active")):
		TestUtils.fail(self, TEST_NAME, "wind_boost not activated after interacting with viento")
		return
