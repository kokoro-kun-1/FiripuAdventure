...
const TEST_NAME: String = "COQUIMBO_TEST"
const COQUIMBO_SCENE := "res://scenes/levels/level_coquimbo.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, COQUIMBO_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var tunel_a: Node = level.get_node_or_null("TunelDeguA")
	var tunel_b: Node = level.get_node_or_null("TunelDeguB")
	var tunel_c: Node = level.get_node_or_null("TunelDeguC")
	var boss: Node = level.get_node_or_null("ChinchillaRobot")
	...
	if kira == null:
		...
		return
	if tunel_a == null or tunel_b == null or tunel_c == null:
		TestUtils.fail(self, TEST_NAME, "Túneles degú not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "ChinchillaRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar túnel degú activa degu_tunnel
	if not firipu.has_method("activate_degu_tunnel"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_degu_tunnel")
		return

	tunel_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("degu_tunnel_active")):
		TestUtils.fail(self, TEST_NAME, "degu_tunnel not activated after interacting with tunel")
		return
