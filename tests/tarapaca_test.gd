...
const TEST_NAME: String = "TARAPACA_TEST"
const TARAPACA_SCENE := "res://scenes/levels/level_tarapaca.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, TARAPACA_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var cristal_a: Node = level.get_node_or_null("CristalAntiespejismoA")
	var cristal_b: Node = level.get_node_or_null("CristalAntiespejismoB")
	var cristal_c: Node = level.get_node_or_null("CristalAntiespejismoC")
	var boss: Node = level.get_node_or_null("LagartoRobot")
	...
	if kira == null:
		...
		return
	if cristal_a == null or cristal_b == null or cristal_c == null:
		TestUtils.fail(self, TEST_NAME, "Cristales anti-espejismo not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "LagartoRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar cristal anti-espejismo activa crystal
	if not firipu.has_method("activate_crystal"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_crystal")
		return

	cristal_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("crystal_active")):
		TestUtils.fail(self, TEST_NAME, "crystal not activated after interacting")
		return
