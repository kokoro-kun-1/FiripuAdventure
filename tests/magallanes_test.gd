...
const TEST_NAME: String = "MAGALLANES_TEST"
const MAGALLANES_SCENE := "res://scenes/levels/level_magallanes.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, MAGALLANES_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var botas_a: Node = level.get_node_or_null("BotasHieloA")
	var botas_b: Node = level.get_node_or_null("BotasHieloB")
	var botas_c: Node = level.get_node_or_null("BotasHieloC")
	var botas_d: Node = level.get_node_or_null("BotasHieloD")
	var boss: Node = level.get_node_or_null("PenguinRobot")
	...
	if kira == null:
		...
		return
	if botas_a == null or botas_b == null or botas_c == null or botas_d == null:
		TestUtils.fail(self, TEST_NAME, "Botas de hielo not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "PenguinRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar botas de hielo activa ice_boots
	if not firipu.has_method("activate_ice_boots"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_ice_boots")
		return

	botas_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("ice_boots_active")):
		TestUtils.fail(self, TEST_NAME, "ice_boots not activated after interacting with botas")
		return
