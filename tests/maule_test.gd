...
const TEST_NAME: String = "MAULE_TEST"
const MAULE_SCENE := "res://scenes/levels/level_maule.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, MAULE_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var tronco_a: Node = level.get_node_or_null("TroncoFlotanteA")
	var tronco_b: Node = level.get_node_or_null("TroncoFlotanteB")
	var tronco_c: Node = level.get_node_or_null("TroncoFlotanteC")
	var boss: Node = level.get_node_or_null("LoroTricahueRobot")
	...
	if kira == null:
		...
		return
	if tronco_a == null or tronco_b == null or tronco_c == null:
		TestUtils.fail(self, TEST_NAME, "Troncos flotantes not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "LoroTricahueRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar tronco flotante activa raft_log
	if not firipu.has_method("activate_raft_log"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_raft_log")
		return

	tronco_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("raft_log_active")):
		TestUtils.fail(self, TEST_NAME, "raft_log not activated after interacting with tronco")
		return
