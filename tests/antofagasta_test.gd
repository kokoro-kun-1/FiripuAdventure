...
const TEST_NAME: String = "ANTOFAGASTA_TEST"
const ANTOFAGASTA_SCENE := "res://scenes/levels/level_antofagasta.tscn"

var level: Node = null

func _ready() -> void:
	print(TEST_NAME, ": starting")
	level = TestUtils.load_main_level(self, TEST_NAME, ANTOFAGASTA_SCENE)
	if level == null:
		return
...
func _run_test() -> void:
	...
	var camanchaca_a: Node = level.get_node_or_null("CamanchacaReveladoraA")
	var camanchaca_b: Node = level.get_node_or_null("CamanchacaReveladoraB")
	var camanchaca_c: Node = level.get_node_or_null("CamanchacaReveladoraC")
	var boss: Node = level.get_node_or_null("EscarabajoCobreRobot")
	...
	if kira == null:
		...
		return
	if camanchaca_a == null or camanchaca_b == null or camanchaca_c == null:
		TestUtils.fail(self, TEST_NAME, "Camanchaca reveladoras not found")
		return
	if boss == null:
		TestUtils.fail(self, TEST_NAME, "EscarabajoCobreRobot boss not found")
		return
	if medal_area == null:
	...
	# 2) Probar camanchaca reveladora activa camanchaca
	if not firipu.has_method("activate_camanchaca"):
		TestUtils.fail(self, TEST_NAME, "Firipu missing activate_camanchaca")
		return

	camanchaca_a.call("interact", firipu)
	await get_tree().process_frame

	if not bool(firipu.get("camanchaca_active")):
		TestUtils.fail(self, TEST_NAME, "camanchaca not activated after interacting")
		return
